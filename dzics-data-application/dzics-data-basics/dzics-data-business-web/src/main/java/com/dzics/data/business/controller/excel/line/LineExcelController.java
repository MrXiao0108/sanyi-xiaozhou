package com.dzics.data.business.controller.excel.line;
...
public class LineExcelController {
    ...
    public void list(@RequestHeader(value = "jwt_token", required = false) @ApiParam(value = "token令牌", required = true) String tokenHdaer,
                     ...
                     LineParmsList lineParmsList, HttpServletResponse response) throws IOException {
        String fileNameBase = "产线管理";
        try {
            lineParmsList.setPage(-1);
            lineParmsList.setLimit(FinalCode.SELECT_SUM_EXCEL);
            Result<List<LineDo>> list = lineService.list(sub, lineParmsList);
            String fileName = URLEncoder.encode(fileNameBase + "-" + System.currentTimeMillis(), "UTF-8").replaceAll("\\+", "%20");
            response.setHeader("Content-Disposition", "attachment;filename=" + fileName + ".xlsx");
            response.setContentType(MediaType.APPLICATION_OCTET_STREAM_VALUE + ";charset=UTF-8");
            EasyExcel.write(response.getOutputStream(), LineDo.class).registerWriteHandler(new CustomCellWriteHandler()).sheet(fileNameBase).doWrite(list.getData());
        }catch (Exception e){
