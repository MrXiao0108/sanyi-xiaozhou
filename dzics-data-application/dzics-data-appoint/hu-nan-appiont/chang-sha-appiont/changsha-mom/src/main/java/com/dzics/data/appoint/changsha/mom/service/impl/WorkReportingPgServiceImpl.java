 ...
 * @Description 销轴抛光MOM 报工
 * @Date 2022/6/16 17:13
 * @Created by NeverEnd
 */
@Service
@Slf4j
public class WorkReportingPgServiceImpl implements WorkReportingService<Boolean, SendPosition> {
    @Autowired
    ...
    public Boolean sendWorkReportingData(SendPosition parms, String momWaitWorkReportId) {
        ...
        List<String> list = Arrays.asList("72", "76", "79");
        if (list.contains(stationCode)) {
        //找到当前二维码绑定的订单，拿此订单进行报工
            MomOrderQrCode qrMomOrder = qrCodeService.getQrMomOrder(parms.getQrCode(), parms.getOrderNo(), parms.getLineNo());
            if(qrMomOrder == null){
                log.error("WorkReportingPgServiceImpl [sendWorkReportingData] 处理报工数据，当前二维码：{}处于无订单状态下生产，无法进行报工",parms.getQrCode());
                return false;
            }
            MomOrder momOrder = momOrderService.getById(qrMomOrder.getProTaskOrderId());
            log.info("WorkReportingPgServiceImpl [sendWorkReportingData] WipOrderNo{}", momOrder.getWiporderno());
//        封装参数 发送报工请求
            String b = WorkReport(momOrder, parms.getProductionTime(), parms.getOutInputType(), parms.getQrCode(), stationManagement.getDzStationCode());
            if ("success".equals(b)) {
                return true;
            } else {
                ...
                log.info("WorkReportingPgServiceImpl [sendWorkReportingData] 报工记录变动:{}", JSON.toJSONString(report));
            }
        }
//        else {
...
//        }
    ...
    public String WorkReport(MomOrder momOrder, Date productionTime, String outInputType, String qrCode, String baoGongBianMa) {
        log.info(" mom---> {}", JSONUtil.toJsonStr(momOrder));
        ...
        String progressType = outInputType.equals(QrCode.QR_CODE_IN) ? MomProgressType.START.getCode() : MomProgressType.COMPLETE.getCode();

        String key = ChangShaRedisKey.MOM_BAO_GONG_REQ_ID + qrCode;
        if (MomProgressType.START.getCode().equals(progressType)) {
            vo.setReqId(redisUniqueID.getkey());
            redisUtil.set(key, vo.getReqId(), (long) 60 * 60);
        } else {
            String reqId = redisUtil.get(key);
            if (StringUtils.isEmpty(reqId)) {
                log.error("WorkReportingPgServiceImpl [WorkReport] reqId is null");
            }
            vo.setReqId(reqId);
        }
