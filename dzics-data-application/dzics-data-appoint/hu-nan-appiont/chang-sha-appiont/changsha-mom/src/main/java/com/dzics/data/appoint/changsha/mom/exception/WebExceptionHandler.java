...
public class WebExceptionHandler {
    ...
    public Result handleBindException(MethodArgumentNotValidException ex, HttpServletRequest req) {
        log.error("WebExceptionHandler [handleBindException] 传递参数异常 url {},异常信息:{}", req.getRequestURI(), ex.getBindingResult().getFieldError().getDefaultMessage());
...
//        String collect = ex.getBindingResult().getFieldErrors().stream().map(err -> err.getDefaultMessage()+"|").collect(Collectors.joining());
    ...
    public Result unexpectedTypeException(UnexpectedTypeException ex, HttpServletRequest req) {
        log.error("WebExceptionHandler [unexpectedTypeException] 传递参数异常 url {},异常信息:{}", req.getRequestURI(), ex.getMessage());
    ...
    public Result customerException(CustomException e, HttpServletRequest req) {
        log.error("WebExceptionHandler [customerException] 自定义错误异常 url {},异常信息:{}", req.getRequestURI(), e.getMessage());
    ...
    public Result exception(Exception e, HttpServletRequest req) {
        log.error("WebExceptionHandler [exception] 未知异常 url {},异常信息:{}", req.getRequestURI(), e.getMessage(), e);
    ...
    public Result httpMessageNotReadableException(HttpMessageNotReadableException e, HttpServletRequest req) {
        log.error("WebExceptionHandler [httpMessageNotReadableException] JSON 参数错误 url {},异常信息:{}", req.getRequestURI(), e.getMessage());
    ...
    public Result customerException(CustomWarnException e, HttpServletRequest req) {
        log.warn("WebExceptionHandler [customerException] 警告异常 url {},异常信息:{}", req.getRequestURI(), e.getMessage());
        return new Result(e.getCode(), e.getMessage());
    }
}
