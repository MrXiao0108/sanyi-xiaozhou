 ...
 * @Description 检测数据表复合分片算法实现
 * @Date 2022/3/1 11:31
 * @Created by NeverEnd
 */
@Slf4j
public class ShardingAlgorithmWorkpiece implements ComplexKeysShardingAlgorithm<String> {

    /**
     * Sharding.
     * <p>
     * order_no,line_no,product_no
     *
    ...
    public Collection<String> doSharding(Collection<String> availableTargetNames, ComplexKeysShardingValue<String> shardingValue) {
        ...
        if (MapUtils.isNotEmpty(collectionMap)) {
            String order_code = (String) collectionMap.get("order_no").toArray()[0];
            String line_no = (String) collectionMap.get("line_no").toArray()[0];
            String product_no = (String) collectionMap.get("product_no").toArray()[0];
            String sh = order_code + line_no + product_no;
            int sha = Math.abs(sh.hashCode() % 20) + 1;
            strings.add("dz_workpiece_data_" + sha);
        }
        return strings;
    }
}
